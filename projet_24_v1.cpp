...
//
 ...
 if (argc == 2)
  ...
  double		t_optimised_median  = 0.0;
  double		t_mycode_grad    = 0.0;
  double		t_optimised_grad    = 0.0;
  double		temp  = 0.0;
...
// mesure de temps d'ex̩cution de cette partie :
  ...
  // calcul de la mediane - librairie OpenCV
    ...
    t_mycode_median = t_mycode_median + temp;
          printf("-------------------------------------\n");
     printf("Median MyCode \t %g \n", temp);gettimeofday(&t0,NULL);
    // **********************************************************************
    gettimeofday(&t0,NULL);
    my_median_optimized(im_in_gray, im_blurred,n);
    gettimeofday(&t1,NULL);
    temp  = (double)((t1.tv_sec-t0.tv_sec)*1000000LL + t1.tv_usec-t0.tv_usec)/1000.0;
    t_optimised_median = t_optimised_median + temp;
          printf("-------------------------------------\n");
     printf("Median optimised \t %g \n", temp);
    // mesure de temps d'ex̩cution de cette partie :
    gettimeofday(&t0,NULL);
    // ********************************************************************** 
    ...
    gettimeofday(&t1,NULL);
    temp  = (double)((t1.tv_sec-t0.tv_sec)*1000000LL + t1.tv_usec-t0.tv_usec)/1000.0;
    t_mycode_grad  = t_mycode_grad + temp;
      printf("Sobel MyCode \t %g \n", temp);gettimeofday(&t0,NULL);
          gettimeofday(&t0,NULL);
    // ********************************************************************** 
    ...
    gettimeofday(&t1,NULL);
    temp  = (double)((t1.tv_sec-t0.tv_sec)*1000000LL + t1.tv_usec-t0.tv_usec)/1000.0;
    t_optimised_grad  = t_optimised_grad + temp;
      printf("Sobel MyCode \t %g \n", temp);
    // ********************************************************************** 
  ...
  // Emprimer le temps d'ex̩uction moyen par version
   ...
   t_optimised_median /= no_measures;
   t_optimised_grad /= no_measures;
   ...
   printf(" Mean median time optimized %f ms\n",t_optimised_median);
